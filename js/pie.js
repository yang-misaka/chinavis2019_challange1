function pie(dayX) {
	d3.csv("data/classify1.csv", function (error, data) {
		...
		// 数据准备
		let dataset = [{ label: '工作人员', num: work.length }, { label: '记者', num: media.length }, { label: '普通观众', num: audi.length },
		{ label: '参赛人员', num: player.length }, { label: '特殊嘉宾', num: vip.length }];
		...
		// 颜色比例尺
		var colorScale = ['#6ab0b8','#e98f6f','#334b5c','#9fdabf','#c23531'];
		// 创建饼状图
			...
			function drawList(page) {
				...
				d3.csv(dayX_results[dayX], function (error, data) {
					...
					function binarySearch(id) {
						...
						for (let index = left; data[index].id == id; index++) {
